...
| What | Where | Why |
|------|-------|-----|
| Compose project (this repo) | `~/homelab` — the Forgejo stack lives in `infra/forgejo/` | You edit/redeploy it as your normal user; no root needed to run `docker compose`. |
...
3. **`init-forgejo.sh` — project-specific setup.** Does the remaining Part 0
   ...
   - Tells the **host Docker daemon** to allow the plain-HTTP Forgejo registry by
     ...
     with `REGISTRY_ADDR=host:port scripts/init-forgejo.sh`, and use the same address as
