...
- **apps VM** — Coolify (self-hosted PaaS). Coolify owns its own Docker
  ...
  runs and why, never how. It deliberately adds no rows to the `docs/`
...
Homepage, monitoring, Traefik, Uptime Kuma and Vaultwarden — so every tier-1
and tier-2 row in `docs/roadmap/backup.md` has a `backup.sh` and a `restore.sh`
beside its compose file. **Homepage's is the smallest**: `include_env` and
nothing else, because its compose and its whole `config/` tree are tracked in
this repo and it has no `/opt` directory at all. Its `.env` is the only thing a
checkout does not already carry, and it is not regenerable either — every value
is a widget credential minted by hand in another service
(`docs/homepage-widgets.md`).
...
10. `scripts/init-uptime-kuma.sh` — creates `/opt/uptime-kuma`, symlinks the
   stack. It has **no `.env` and no `.env.example`**: Kuma has no
...
11. `scripts/init-homepage.sh` — ensures the `proxy` network, seeds `.env` from
   `.env.example`, and symlinks the stack. Homepage has **no `/opt/<stack>` data
   directory at all**: its configuration is the git-tracked YAML in
   `infra/homepage/config`, bind-mounted read-only. Nothing in its `.env` is
   **generated** — every value is a widget credential minted by hand in another
   service, so the script seeds placeholders and the guide sends you to
   `docs/homepage-widgets.md`. The config mount being read-only has a
   consequence worth knowing: Homepage copies a skeleton file in when one is
   missing and `exit(1)`s when that copy fails, so **every** config file it
   looks for must exist — several ship as stubs. Last stack on this VM: it links
   every one above it and nothing links it.
...
- **`.env` is gitignored**; every stack whose `.env` holds hand-filled values
  ...
  script, so it ships none; Uptime Kuma has no `.env` at all).
  Secrets
...
The **registries** centralize the manual operations that live outside the repo:
...
Kuma monitor, grouped by stack) and `homepage-widgets.md` (every Homepage widget
that needs a credential, and where to mint it). Guides link the registries
instead of duplicating the lists — a new hostname, SSO app, monitor or widget
credential gets its registry row first, and per-service values should never be
repeated inline in a guide.

Each of them carries `**Runs on:** … — registry, not a build step`, and each
lists its **deliberate absences** alongside its entries, because a registry that
...
and Home Assistant have no SSO entry, Kuma does not monitor itself or the
hypervisor, and Proxmox and the Traefik dashboard have widgets that are
deliberately unused. When adding a service, decide about every registry and say
so in each.
