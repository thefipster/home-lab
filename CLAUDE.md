...
1. `scripts/init-docker.sh` — installs Docker Engine + compose plugin (Ubuntu),
...
2. `scripts/init-unattended-upgrades.sh` — the only init script that is **not**
   about a stack and the only one meant for **both** VMs (the apps VM gets its
   Docker from Coolify's installer, so `init-docker.sh` never runs there).
   Writes `/etc/apt/apt.conf.d/20auto-upgrades` plus a `52homelab-…` drop-in
   numbered above the distro's `50unattended-upgrades` so it wins, and enables
   `apt-daily{,-upgrade}.timer`. Two deliberate choices: the origins are
   **security-only** — the `#clear` directives in the drop-in are apt-config
   ...
   override per run. Order relative to `init-docker.sh` is free, but after is
   preferred — apt does TLS, so it wants the clock fix first.
3. `scripts/init-traefik.sh` — creates the `proxy` network + ACME dir, seeds
   `.env` from `.env.example`. The entrypoint-level `tls.domains` makes
   Traefik request the wildcard cert at startup — no router needed.
4. `scripts/init-authentik.sh` — creates `/opt/authentik`, generates secrets
...
5. `scripts/init-dockge.sh` — copies the compose to `/opt/stacks/dockge`,
   ...
   **only** init script that does — **starts the stack itself**, so its guide
   ...
   from the web UI.
6. `scripts/init-forgejo.sh` — creates `/opt/forgejo` data tree, seeds `.env`
   (generates `FORGEJO_DB_PASSWORD`, records `DOCKER_GID`), symlinks the stack
   into `/opt/stacks`.
7. `scripts/init-monitoring.sh` — creates `/opt/monitoring`, chowns each data
   ...
   verified first.
8. `scripts/init-uptime-kuma.sh` — creates `/opt/uptime-kuma`, symlinks the
