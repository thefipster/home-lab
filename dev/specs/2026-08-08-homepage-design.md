...
- **The Kuma widget reads a status page, not an API.** Uptime Kuma has no full
  ...
  [docs/uptime-kuma-monitors.md](../../docs/reference/uptime-kuma-monitors.md); this design adds
...
| Registry | Row |
|---|---|
| [dns-records.md](../../docs/reference/dns-records.md) | `home.thefipster.de` → `infra ip`. An exact record, like `git.` and `dockge.` — the name wants the infra VM, which the wildcard would not deliver. |
| [sso-applications.md](../../docs/reference/sso-applications.md) | A third member of the forward-auth section, beside Dockge and the Traefik dashboard. `infra/authentik/compose.yaml` gains a pair of `ak-outpost-homepage` router labels matching the Homepage host plus the `/outpost.goauthentik.io/` path prefix, exactly as it already does for `dockge` and `traefik`. |
| [uptime-kuma-monitors.md](../../docs/reference/uptime-kuma-monitors.md) | **One** monitor: Docker → `homepage-homepage-1`. **No Web monitor** — forward-auth gated, the same 302 reasoning the registry already applies to Dockge and the Gateway. Plus a row recording the existing `homelab` status page. |
